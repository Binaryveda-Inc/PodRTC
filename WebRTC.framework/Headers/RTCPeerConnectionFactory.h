...
RTC_OBJC_EXPORT
@protocol AudioBufferDelegate <NSObject>
-(void)onAudioBuffer:(AudioBuffer*)buffer isLocal:(BOOL)isLocal;
@end
...
+ (void)setDelegate:(nullable id<RTC_OBJC_TYPE(AudioBufferDelegate)>)delegate;

+ (void)sendAudioBuffer:(AudioBuffer *)audio_buffer isLocal:(BOOL)isLocal;
